...
{
?>
<form action="notice.php?act=addok" method="post" name="form1" onSubmit="return doCheck();">
  <table width="98%" align="center" border="1" cellspacing="0" cellpadding="4" class="lanyubk" style="border-collapse: collapse">
    <tr class="lanyuss">
      <td height="20"  colspan="2">公告发布</td>
    </tr>
    <tr class="lanyuds">
    ...
    <tr class="lanyuds">
      ...
      <td height="46" align="left"><textarea name="content" style="display:none"></textarea>
        ...
        <!-- <input type="text" name="anyfield" value="" ID="WebEditor1" src="../WebEditor/ewebeditor.htm?id=content&style=coolblue" frameborder="0" scrolling="no" width="550" HEIGHT="350">-->
    </tr>
    <tr class="lanyuds">
    ...
    <tr class="lanyuqs">
      <td height="23" align="right" colspan="2"><p align="center">
          ...
          &nbsp;&nbsp;&nbsp;&nbsp;<input type="reset" name="Submit2" value="取消" onClick="document.location.href='notice.php?act=list';">
        </p></td>
    </tr>
  </table>
</form>
...
elseif($_GET['act'] == "addok")
{
	...
	$db->insert("INSERT INTO `notice` ( `title` , `author` , `show` , `ntime` , `content` , `hits`) VALUES('".$title."','".$author."','".$show."','".$ntime."','".$content."','".$hits."')");
	echo "<meta http-equiv=\"refresh\" content=\"0;URL=notice.php?act=list\">";
	exit;
}
//公告列表
...
<table width="98%" align="center" border="1" cellspacing="0" cellpadding="4" class="lanyubk" style="border-collapse: collapse">
  <tr class="lanyuss">
    <td height="20"  colspan="3">公告管理</td>
  </tr>
  <tr class="lanyuds">
...
?>
  <tr class="lanyuds">
    <td height="23"  width="12%"><div align="center"><?php echo $row[nid]; ?></div></td>
    <td width="62%" height="23"><div align="center"><?php echo $row[title]; ?></div></td>
    <td width="26%"><div align="center"><a href="notice.php?id=<?php echo $row[nid]; ?>&act=del" onClick="{if(confirm('确定要删除吗？\n\n删除之后就无法还原!')){return true;}return false;}">删除</a>&nbsp;&nbsp;<a href="notice.php?id=<?php echo $row[nid]; ?>&act=edit">修改</a></div></td>
  </tr>
<?php
}
?>  
...
elseif($_GET['act'] == "edit")
{
	...
	$row = $db->query("SELECT * FROM notice WHERE nid='".$_GET['id']."'");
    //mysql_query("set names 'gb2312'");//这就是指定数据库字符集，一般放在连接数据库后面就系了
   $edit=$db->getarray($row);
?>
<form action="notice.php?act=editok" method="post" name="form1" onSubmit="return doCheck();">
  <table width="98%" align="center" border="1" cellspacing="0" cellpadding="4" class="lanyubk" style="border-collapse: collapse">
    <tr class="lanyuss">
      <td height="20"  colspan="2">公告修改</td>
    </tr>
    <tr class="lanyuds">
      <td height="23"  width="12%" align="right"><font color="#FF0000">*</font>标题:</td>
    ...
    <tr class="lanyuqs">
      <td height="23" align="right" colspan="2"><p align="center">
	      <input type="hidden" name="id" value="<?=$edit[nid]?>">
...
elseif($_GET['act'] == "editok")
{
    ...
    $content=$_POST['content'];
    $hits=intval($_POST['hits']);
	$db->update("update notice set title='".$title."',author='".$author."',`show`='".$show."',ntime='".$ntime."',content='".$content."',hits='".$hits."' where nid='".$_POST['id']."'");  
    echo "<meta http-equiv=\"refresh\" content=\"0;URL=notice.php?act=list\">";
    exit;
}
//删除部分
elseif($_GET['act'] == "del")
{
	...
	$rs = $db->query("delete from notice where nid='".$_GET['id']."'");
    echo "<meta http-equiv=\"refresh\" content=\"0;URL=notice.php?act=list\">";
    exit;
}
?>
