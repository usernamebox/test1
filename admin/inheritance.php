...
function seeme()
{
    ...
    <tr class="lanyuqs">
      <td height="23" align="right" colspan="2"><p align="center">
          ...
          &nbsp;&nbsp;&nbsp;&nbsp;
          <input type="reset" name="Submit2" value="取消" onclick="document.location.href='inheritance.php?act=list';">
        </p></td>
    </tr>
  </table>
</form>
...
elseif($_GET['act'] == "addok")
{
	...
	$result=$db->query("select * from inheritance");
	$db->insert("INSERT INTO `inheritance` ( `cat` , `title` , `author`, `show` , `ntime` , `content` , `hits`, `istop`, `photo`) VALUES('".$cat."','".$title."','".$author."','".$show."','".$ntime."','".$content."','".$hits."','".$istop."','".$photo."')");
	Error("添加成功！","inheritance.php?act=list");
    echo "添加成功";
}
//列表
...
$num=30;  //每页记录数.
$total=$db->getcount("select * from inheritance");
...
<table width="98%" align="center" border="1" cellspacing="0" cellpadding="4" class="lanyubk" style="border-collapse: collapse">
  <tr class="lanyuss">
    <td height="20"  colspan="4">传承管理</td>
  </tr>
  <tr class="lanyuds">
...
<?php  
$result=$db->query("select * from inheritance order by id desc limit $offset,$num");
//mysql_query("set names 'gb2312'");//这就是指定数据库字符集，一般放在连接数据库后面就系了
...
?>
  <tr class="lanyuds">
    ...
    <td width="18%" height="23"><div align="center"><?php if($row[cat]==1)
	 ...
	 ?></div></td>
    <td width="57%" height="23"><div align="center"><?=$row[title]?><?=($row[istop]==1)?"(头条)":""?></div></td>
    <td width="13%"><div align="center"><a href="inheritance.php?id=<?=$row[id]?>&act=del" onClick="{if(confirm('确定要删除吗？\n\n删除之后就无法还原!')){return true;}return false;}">删除</a>&nbsp;&nbsp;<a href="inheritance.php?id=<?=$row[id]?>&act=edit">修改</a></div></td>
  </tr>
<?php
}
?>  
...
elseif($_GET['act'] == "edit")
{
	if(empty($_GET['id']))
	{
		Error("非法操作...","inheritance.php?act=list");
		die();
	}
	$row = $db->query("SELECT * FROM inheritance WHERE id='".$_GET['id']."'");
    //mysql_query("set names 'gb2312'");//这就是指定数据库字符集，一般放在连接数据库后面就系了
   $edit=$db->getarray($row);
?>
<form action="inheritance.php?act=editok" method="post" name="form1" onSubmit="return doCheck();">
  <table width="98%" align="center" border="1" cellspacing="0" cellpadding="4" class="lanyubk" style="border-collapse: collapse">
    <tr class="lanyuss">
      <td height="20"  colspan="2">传承修改</td>
    </tr>
	<tr class="lanyuds">
      <td height="23"  width="12%" align="right"><font color="#FF0000">*</font>分类:</td>
      <td height="23" align="left"><select name="cat">
	  <option value="1" <?=($edit[cat]==1?"selected":"")?>>人生导师</option>
	  <option value="2" <?=($edit[cat]==2?"selected":"")?>>创业导航</option>
	  </select></td>
    </tr>
    <tr class="lanyuds">
      <td height="23"  width="12%" align="right"><font color="#FF0000">*</font>标题:</td>
...
elseif($_GET['act'] == "editok")
{
	if(empty($_POST['id']))
	{
		Error("非法操作...","inheritance.php?act=edit");
		die();
	}
	$cat=$_POST['cat'];
	$istop = isset($_POST["istop"]) ? 1 : 0;
    ...
    $hits=intval($_POST['hits']);
	$photo=SafeHtml($_POST['photo']);
	$db->update("update inheritance set cat='".$cat."',title='".$title."',author='".$author."',`show`='".$show."',ntime='".$ntime."',content='".$content."',hits='".$hits."',istop='".$istop."',photo='".$photo."' where id='".$_POST['id']."'");  
	Error("修改成功！","inheritance.php?act=list");
    echo "修改成功";
}
//删除部分
elseif($_GET['act'] == "del")
{
	...
	$rs = $db->query("delete from inheritance where id='".$_GET['id']."'");
	Error("删除成功！","inheritance.php?act=list");
    echo "删除成功";
}
?>
</body>
