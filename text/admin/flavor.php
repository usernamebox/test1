...
{
?>
<form action="flavor.php?act=addok" method="post" name="form1" onSubmit="return doCheck();">
  <table width="98%" align="center" border="1" cellspacing="0" cellpadding="4" class="lanyubk" style="border-collapse: collapse">
    <tr class="lanyuss">
      <td height="20"  colspan="2">风采发布</td>
    </tr>
    <tr class="lanyuds">
...
function seeme()
{
    ...
    <tr class="lanyuqs">
      <td height="23" align="right" colspan="2"><p align="center">
          ...
          &nbsp;&nbsp;&nbsp;&nbsp;
          <input type="reset" name="Submit2" value="取消" onclick="document.location.href='flavor.php?act=list';">
        </p></td>
    </tr>
  </table>
</form>
...
elseif($_GET['act'] == "addok")
{
	...
	$result=$db->query("select * from flavor");
	$db->insert("INSERT INTO `flavor` ( `title` , `author` , `show` , `keywords` , `ntime` , `content` , `hits`, `istop`, `photo`) VALUES('".$title."','".$author."','".$show."','".$keywords."','".$ntime."','".$content."','".$hits."','".$istop."','".$photo."')");
	Error("添加成功！","flavor.php?act=list");
    echo "添加成功";
}
//列表
...
<table width="98%" align="center" border="1" cellspacing="0" cellpadding="4" class="lanyubk" style="border-collapse: collapse">
  <tr class="lanyuss">
    <td height="20"  colspan="3">风采管理</td>
  </tr>
  <tr class="lanyuds">
...
?>
  <tr class="lanyuds">
    ...
    <td width="62%" height="23"><div align="center"><?php echo $row[title]; ?><?=($row[istop]==1)?"(头条)":""?></div></td>
    <td width="26%"><div align="center"><a href="flavor.php?id=<?php echo $row[id]; ?>&act=del" onClick="{if(confirm('确定要删除吗？\n\n删除之后就无法还原!')){return true;}return false;}">删除</a>&nbsp;&nbsp;<a href="flavor.php?id=<?php echo $row[id]; ?>&act=edit">修改</a></div></td>
  </tr>
<?php
}
?>  
...
elseif($_GET['act'] == "edit")
{
	if(empty($_GET['id']))
	{
		Error("非法操作...","flavor.php?act=list");
		die();
	}
	$row = $db->query("SELECT * FROM flavor WHERE id='".$_GET['id']."'");
    //mysql_query("set names 'gb2312'");//这就是指定数据库字符集，一般放在连接数据库后面就系了
   $edit=$db->getarray($row);
?>
<form action="flavor.php?act=editok" method="post" name="form1" onSubmit="return doCheck();">
  <table width="98%" align="center" border="1" cellspacing="0" cellpadding="4" class="lanyubk" style="border-collapse: collapse">
    <tr class="lanyuss">
      <td height="20"  colspan="2">风采修改</td>
    </tr>
    <tr class="lanyuds">
      <td height="23"  width="12%" align="right"><font color="#FF0000">*</font>标题:</td>
...
elseif($_GET['act'] == "editok")
{
	if(empty($_POST['id']))
	{
		Error("非法操作...","flavor.php?act=edit");
		die();
	}
	$istop = isset($_POST["istop"]) ? 1 : 0;
    $title=SafeHtml($_POST['title']);
    ...
    $hits=intval($_POST['hits']);
	$photo=SafeHtml($_POST['photo']);
	$db->update("update flavor set title='".$title."',author='".$author."',`show`='".$show."',`keywords`='".$keywords."',ntime='".$ntime."',content='".$content."',hits='".$hits."',istop='".$istop."',photo='".$photo."' where id='".$_POST['id']."'");  
	Error("修改成功！","flavor.php?act=list");
    echo "修改成功";
}
//删除部分
elseif($_GET['act'] == "del")
{
	...
	$rs = $db->query("delete from flavor where id='".$_GET['id']."'");
	Error("删除成功！","flavor.php?act=list");
    echo "删除成功";
}
?>
</body>
