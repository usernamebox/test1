...
function seeme()
{
    ...
    <tr class="lanyuqs">
      <td height="23" align="right" colspan="2"><p align="center">
          ...
          &nbsp;&nbsp;&nbsp;&nbsp;
          <input type="reset" name="Submit2" value="取消" onClick="document.location.href='active.php?act=list';">
        </p></td>
    </tr>
  </table>
</form>
...
elseif($_GET['act'] == "addok")
{
	...
	Error("添加成功！","active.php?act=list");
    echo "添加成功";
}
//列表
...
<table width="98%" align="center" border="1" cellspacing="0" cellpadding="4" class="lanyubk" style="border-collapse: collapse">
  <tr class="lanyuss">
    <td height="20"  colspan="4">活动管理</td>
  </tr>
  <tr class="lanyuds">
...
?>
  <tr class="lanyuds">
    ...
    <td width="13%"><div align="center"><a href="active.php?id=<?=$row[id]?>&act=del" onClick="{if(confirm('确定要删除吗？\n\n删除之后就无法还原!')){return true;}return false;}">删除</a>&nbsp;&nbsp;<a href="active.php?id=<?=$row[id]?>&act=edit">修改</a></div></td>
  </tr>
<?php
}
?>  
...
elseif($_GET['act'] == "edit")
{
	if(empty($_GET['id']))
	{
		Error("非法操作...","active.php?act=list");
		die();
	}
...
<form action="active.php?act=editok" method="post" name="form1" onSubmit="return doCheck();">
  <table width="98%" align="center" border="1" cellspacing="0" cellpadding="4" class="lanyubk" style="border-collapse: collapse">
    <tr class="lanyuss">
      <td height="20"  colspan="2">活动修改</td>
    </tr>
	<tr class="lanyuds">
      <td height="23"  width="12%" align="right"><font color="#FF0000">*</font>分类:</td>
      <td height="23" align="left"><select name="cat">
	  <option value="1" <?=($edit[cat]==1?"selected":"")?>>讲坛</option>
	  <option value="2" <?=($edit[cat]==2?"selected":"")?>>对话</option>
	  <option value="3" <?=($edit[cat]==3?"selected":"")?>>沙龙</option>
	  <option value="4" <?=($edit[cat]==4?"selected":"")?>>年会</option>
	  </select></td>
    </tr>
    <tr class="lanyuds">
      <td height="23"  width="12%" align="right"><font color="#FF0000">*</font>标题:</td>
...
elseif($_GET['act'] == "editok")
{
	if(empty($_POST['id']))
	{
		Error("非法操作...","active.php?act=edit");
		die();
	}
	$cat=$_POST['cat'];
	$istop = isset($_POST["istop"]) ? 1 : 0;
	...
	Error("修改成功！","active.php?act=list");
    echo "修改成功";
}
//删除部分
elseif($_GET['act'] == "del")
{
	...
	Error("删除成功！","active.php?act=list");
    echo "删除成功";
}
?>
</body>
